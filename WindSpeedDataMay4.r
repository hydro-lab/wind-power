...
monthly.wind.towers <- ytower %>% 
     ...
     group_by(ym) %>%
     summarise(ws_mean=mean(WS_ms), ws_sd=sd(WS_ms), ws_sd_means=mean(ws_sd))

daily.wind <- ytower %>% 
     mutate(d=as_date(time_et)) %>%
     group_by(d) %>%
     summarise(ws_mean=mean(WS_ms), ws_sd=sd(WS_ms))
...
november22.wind<- y %>%
     ...
     filter(time_et<=ymd_hms("2022-11-30 23:45:00")) %>%
     summarise(ws_mean=mean(WS_ms_Avg), ws_sd=sd(WS_ms_Avg))
